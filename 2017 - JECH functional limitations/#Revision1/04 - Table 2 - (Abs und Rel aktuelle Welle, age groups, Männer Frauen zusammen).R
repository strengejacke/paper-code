...
source("04x - rd_rr_by_wave_and_age.R")
...
fit <- glm(hlthhmp_d ~ aquinc_med + agea + gndr_n,
           ...
           data = filter(ess, wave == welle, age_dicho2 == alter))

mf <- model.frame(fit)
low <- predict(fit, newdata = data.frame(aquinc_med = "1", agea = mean(mf$agea), gndr_n = mean(mf$gndr_n)), type = "response")
hi <- predict(fit, newdata = data.frame(aquinc_med = "2", agea = mean(mf$agea), gndr_n = mean(mf$gndr_n)), type = "response")

abs_inequ_all <- 100 * (low - hi)
rel_inequ_all <- low / hi

results <- rd_rr_by_wave_and_gender(welle, alter)
...
sjt.df(bind_rows(d1, d2), describe = F)
