...
# ------------------------------------------------------------------
for (i in 1:7) {
  for (j in unique(ess_d$country)) {
    d1 <- subset(ess_d, subset = (country == j & essround == i))
...
# ------------------------------------------------------------------
dummy1$`Income, missing`[dummy1$country == "France"] <- (ess_d %>%
                                                           filter(country == "France", wave != 1) %>%
                                                           summarise(`Income, missing` = round(100 * (sum(is.na(aquinc_med)) / length(aquinc_med)))))[[1]]

dummy1$`Income, missing`[dummy1$country == "Ireland"] <- (ess_d %>%
                                                            filter(country == "Ireland", wave != 1) %>%
                                                            summarise(`Income, missing` = round(100 * (sum(is.na(aquinc_med)) / length(aquinc_med)))))[[1]]

dummy1$`Income, missing`[dummy1$country == "Portugal"] <- (ess_d %>%
                                                             filter(country == "Portugal", wave != 5) %>%
                                                             summarise(`Income, missing` = round(100 * (sum(is.na(aquinc_med)) / length(aquinc_med)))))[[1]]

dummy1$`Income, missing`[dummy1$country == "Hungary"] <- (ess_d %>%
