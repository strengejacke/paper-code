...
source("04x - rd_rr_by_wave_and_gender_and_age.R")
...
fit <- glm(hlthhmp_d ~ aquinc_med + agea,
           ...
           data = filter(ess, wave == welle, gndr_n == geschlecht, age_dicho2 == alter))
...
results <- rd_rr_by_wave_and_gender(welle, geschlecht, alter)
