rd_rr_by_wave_and_gender <- function(welle = 7, geschlecht = 1, alter = 0) {
  ...
  for (i in seq_len(nrow(ess_nested))) {
    x <- filter(ess_nested$data[[i]], gndr_n == geschlecht, age_dicho2 == alter)
    dummy <- na.omit(data.frame(x$hlthhmp_d, x$aquinc_med, x$agea))
    if (nrow(dummy) > 2) {
  ...
  ess_nested$abs_inequ <- unlist(lapply(ess_nested$model.glm, function(x) {
    ...
    mf <- model.frame(x)
    low <- predict(x, newdata = data.frame(aquinc_med = "1", agea = mean(mf$agea)), type = "response")
    hi <- predict(x, newdata = data.frame(aquinc_med = "2", agea = mean(mf$agea)), type = "response")
    return(100 * (low - hi))
  }))
